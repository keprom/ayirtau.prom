...
echo "<tr><td>".anchor_form("billing/billing_point_info/{$r->id}","Информация по точкам учета")."</td><td>".anchor_form("billing/full_close_firm/{$r->id}","Закрыть/открыть  предприятие'><br /> (Закрыть временно <input type=checkbox name=vremenno>) <input type=hidden value='")."</td></tr>"; 
echo "<tr><td>".anchor_form("billing/pre_graph/{$r->id}","График потребления")."</td><td>".anchor_form("billing/add_akt_with_tariff/{$r->id}","Добавить тарифицированный акт")."</td></tr>"; 
echo "<tr><td>".anchor_form("billing/docs_register/{$r->id}","Регистрация документов")."</td><td></td></tr>"; 
echo "</table><br>";
