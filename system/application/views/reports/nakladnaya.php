...
function datetostring3($date)
{
	...
	if ($d['1']==1) {$d['1']='января';}
	if ($d['1']==2) {$d['1']='февраля';}
	if ($d['1']==3) {$d['1']='марта';}
	if ($d['1']==4) {$d['1']='апреля';}
	if ($d['1']==5) {$d['1']='мая';}
	if ($d['1']==6) {$d['1']='июня';}
	if ($d['1']==7) {$d['1']='июля';}
	if ($d['1']==8) {$d['1']='августа';}
	if ($d['1']==9) {$d['1']='сентября';}
	if ($d['1']==10) {$d['1']='октября';}
	if ($d['1']==11) {$d['1']='ноября';}
	if ($d['1']==12) {$d['1']='декабря';}
 	...
 	<p><o:p> </o:p>
		<table border=1 cellspacing=0 cellpadding=0  width=30% align=right >
			...
			<tr>
				...
				<td align=center><?php if (strlen($schet_nakl)==0){ echo ($schetfactura_date->id);} else {echo $schet_nakl;}?></td>
				<td align=center><?php 
		...
		<td align=center><font size=5>Накладная на отпуск товаров на сторону</font></td>
		<tr><td>	
		<p><o:p> </o:p>
		...
		<table border=1 cellspacing=0 cellpadding=0 width=100%>
			<tr>
				...
				<td rowspan=2 align=center>Цена за единицу, в тенге</td>
...
?>	
			...
			<tr>
				...
				<td align=center></td>
				<td align=center><?php echo f_d($s2->kvt);$i_kvt+=f_d($s2->kvt); ?></td>
				<td align=center><?php echo f_d((($s2->tariff_value)*0.12)+$s2->tariff_value);?></td>
				<td align=center><?php 
	...
	?></td>
			...
			<tr>
				<td colspan=5></td>
				<td align=center><b><?php echo f_d($i_kvt);?></b></td>
		...
		<td >Всего отпущено товара (количество прописью): <?php echo kvt2str($i_kvt);?></td>
		<p><o:p> </o:p><tr><tr>
			...
			<tr>
				<td>
					<table border=0 cellspacing=0 width=500px>
						<tr>
								<td width=140px>Отпуск разрешил
								<td colspan=3>  <u><?php echo "Директор ". trim($org->director);?></u></td></td></tr>
								<tr><td ></td>
								<td align=center width=150px> <sup><i>(должность)</i></sup> </td>
								<td align=center> <sup><i>(Ф.И.О.)</i></sup></td>
								<td align=center width=150px> <sup><i> (Подпись)</i></sup></td>
								...
								<td>Отпустил </td>
								<td colspan=3><?php echo "___________________";?></td></tr><tr>
								<td></td>
				...
				<td> 
					<table width=500px>
						<tr>
							<td colspan=3>
								По договору № <u><?php if (strlen($dog2)=='') { echo $firm->dogovor;} else {echo $dog2;} ?></u>
							</td>
						
						</tr><tr>
							<td colspan=3> 
								<u><?php if (strlen($data_dog2)=='') {echo "от ". datetostring3($firm->dogovor_date)." года ";} else {echo $data_dog2;}?></u>
							</td>
						</tr><tr><td><br><br><br><br><br></td></tr><tr>
							<td colspan=3> 
								Товар получил ___________________
							</td>
						</tr><tr>
							<td></td> 
							<td align=center>	
